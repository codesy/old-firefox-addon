...
//  var codesyDomain = require('sdk/simple-prefs').prefs.domain;
  ...
  var codesyHtml = '<hr/>' +
      ...
      '</div>';
	var enabledSitesStr ='' ; //just for clean output outside of loop
  for(var propName in allPrefs) {
			if(propName.substring(0,5) == "addTo") {
				var prefVal = allPrefs[propName] ;
				if ( prefVal !== true ) {
//					console.log("pref " + propName + " is configured but disabled .. ") ;
					continue;
				}
				//assemble the full include parameter from the name of the pref
				var lcase; //lowercase version of the first part of the pref
				...
				enabledSitesStr += "'" + propName + "' enabled, '" + modIncludeStr + "' selector for include "; 
